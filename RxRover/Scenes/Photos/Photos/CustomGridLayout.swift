...
// For more info, see "For a Complex Grid, Define Cell Sizes Explicitly" on the url below
...
class CustomGridLayout: UICollectionViewLayout {
    ...
    private func calculateGeometry(collectionView: UICollectionView) {
        ...
        let headerAttribute = UICollectionViewLayoutAttributes(forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, with: IndexPath(row: 0, section: 0))
        
        headerAttribute.frame = CGRect(x: 0, y: 0, width: cvWidth, height: headerHeight)
        
        cachedAttributes.append(headerAttribute)
        
        // iterate over collection view items
